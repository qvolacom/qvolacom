...
const NavBar: React.FC = () => {
  ...
  useEffect(() => {
    const updateList = () => {
      const storedArray = localStorage.getItem('ProductsList');
      if (storedArray) {
        setProductsList(JSON.parse(storedArray));
      } else {
        console.log("No hay datos en localStorage con la clave 'ProductsList'.");
      }
    };
  ...
  const removeItem = (index: number) => {
    ...
    if (storedArray) {
      ...
      localStorage.setItem('ProductsList', JSON.stringify(array));
          ...
          <div className="absolute md:block min-[320px]:top-[30px] min-[320px]:right-[260px] min-[321px]:top-[30px] min-[321px]:right-[70px] md:right-[20px] lg:right-16 xl:right-20">
            ...
            {cartVisible && (
              ...
              <div className="bg-white 2xl:w-1/5 xl:w-[30%] lg:w-[32%] md:w-[50%] min-[320px]:w-[80%] h-[520px] text-center p-2 border-2 shadow-md absolute top-10 md:right-0 min-[320px]:right-10" style={{animation: "slideRight 0.5s forwards"}}>
                ...
                <ul className="h-[280px] py-2 mt-2 overflow-y-scroll border-dashed border-b-2"></ul>
        
                <div className="flex w-full">
                  <span id="total-price" className="w-full text-end px-5 mt-1">Total: 00.00$</span>
                </div>
